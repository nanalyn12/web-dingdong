...
| 2026-09-08 | 커리큘럼 생성기에 ASSURE 모형 6단계 (배치 14)  | src/lib/assure.ts(+test), curriculum.functions.ts, drizzle/0020, curriculum 라우트 2개, curriculum-pdf-button.tsx, tenant-backup.ts | 지도안이 교수설계 모형 없이 나와 논문·수업 근거로 쓰기 어려웠음. 정규화를 순수 모듈로 빼 화면·PDF·인쇄·백업 네 경로가 한 규칙을 쓰게 하고, 레거시 행 보강 경로를 따로 냄 |
...
| 2026-09-22 | 운영 점검 후속 5건 (구간 표시·스토리북/어휘 비교·편집 버튼·퀴즈 기록·경미 3건) | src/lib/{song-lyrics,lesson-extras,lesson-progress,pdf-report}.ts(+test 5개), generate-lesson 프롬프트, 레슨·학습송·스튜디오 라우트, PDF 버튼 2개, teacher-guide.ts | 순서 맞추기 219곡 전부 `[Verse 1]`이 문제로 섞였음. 스토리북 132쪽·어휘 비교 50개가 저장만 되고 표시 0 — 프롬프트가 구조를 안 정해 21·10가지 모양이 섞여 있어 정규화기로 읽고 프롬프트에 구조를 고정. 퀴즈는 열기만 해도 "학습함"이었음. 스튜디오 번호 ⑤⑨ 누락, 상세 탭 제목·공유 미리보기 "dingdong lms", PDF 중국어 글꼴 혼용 |
| 2026-09-22 | 학습송 상세 하이드레이션 오류 + 문서 언어 ko | src/lib/ssr-consistency.test.ts, \_app.songs.$id.tsx, \_\_root.tsx, curriculum-pdf-button.tsx | 로더가 채운 쿼리 캐시는 서버에만 있어 브라우저 첫 렌더가 `null` → 직접 진입마다 React #418. 로더 반환값을 initialData로 줘 양쪽 첫 렌더를 맞춤. 한국어 앱이 `lang="en"`으로 선언돼 있었음 |
| 2026-09-22 | 화면 한자를 한 간체 글꼴로                      | src/styles.css, src/lib/han-font.test.ts | 앱이 글꼴을 따로 안 정해 한국어 Windows에서 한자가 맑은 고딕(한국 한자)부터 잡히고 간체 전용 글자만 다른 글꼴로 빠져 한 줄에 두 글꼴이 섞였음. 한자 범위만 맡는 local() 별칭을 sans 스택 맨 앞에 둠 — 한글·라틴은 범위 밖 |
