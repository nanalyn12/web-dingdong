...
export function isGuestGenerationBlocked(err: unknown): boolean {
  return (err instanceof Error ? err.message : String(err)).includes(GUEST_GENERATION_BLOCKED);
}
...
async function buildVocabPractice(
  data: z.infer<typeof Input>,
  userId: string | null,
): Promise<VocabPractice> {
  ...
  const safeExamples = (arr: unknown): VocabExample[] =>
    Array.isArray(arr)
      ? arr
...
export const generateVocabPractice = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<VocabPractice> => {
    ...
    const practice = await buildVocabPractice(data, context.userId);
